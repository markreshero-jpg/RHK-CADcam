...
export default function DrillBlockSetup({ machineId }: { machineId: string }) {
  ...
  useEffect(() => {
    let cancelled = false
    async function load() {
      const [blkR, drillR] = await Promise.all([
  ...
  const vbH = M * 2 + (block.bank_y_count - 1) * STEP
  ...
  function spindleNode(bank: 'x' | 'y', position: number) {
    ...
    return (
      <g key={`${bank}${position}`} className="cursor-pointer" onClick={() => setSel({ bank, position })}>
        ...
        <text x={x} y={y + R + 9} textAnchor="middle" fontSize={7} fill="#64748b" className="font-mono">{s?.bit_value ?? 2 ** (position - 1)}</text>
      </g>
    )
  }
    ...
    <div className="py-1">
      ...
      <div className="flex gap-6 flex-wrap">
        ...
        <div className="border border-edge-strong rounded-lg bg-surface-2/40 p-2">
          <svg width={Math.min(vbW, 620)} viewBox={`0 0 ${vbW} ${vbH}`} style={{ maxWidth: '100%', height: 'auto' }}>
            {/* axis guide lines */}
            ...
            {/* bank labels */}
            <text x={cx('x', block.bank_x_count)} y={baseY + R + 22} textAnchor="end" fontSize={9} fill="#64748b">X bank · {block.x_bank_mcode} →</text>
            <text x={baseX - R - 8} y={cy('y', block.bank_y_count)} textAnchor="middle" fontSize={9} fill="#64748b" transform={`rotate(-90 ${baseX - R - 8} ${cy('y', block.bank_y_count)})`}>Y bank · {block.y_bank_mcode} ↑</text>
